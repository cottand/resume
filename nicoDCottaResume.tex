\documentclass[9pt]{developercv}
\usepackage{textcomp}

%----------------------------------------------------------------------------------------

\newcommand{\para}{\vspace{0.4em}}

\begin{document}

%----------------------------------------------------------------------------------------
%	TITLE AND CONTACT INFORMATION
%----------------------------------------------------------------------------------------


    \begin{minipage}[t]{0.45\textwidth} % 45% of the page width for name
        \vspace{-\baselineskip} % Required for vertically aligning minipages

        % If your name is very short, use just one of the lines below
        % If your name is very long, reduce the font size or make the minipage wider and reduce the others proportionately
        \colorbox{accent}{{\HUGE\textcolor{white}{\textbf{{Nico D'Cotta}}}}} % First name

        \vspace{6pt}

        {\huge Backend Software Engineer} % Career or current job title
    \end{minipage}
    \begin{minipage}[t]{0.275\textwidth} % 27.5% of the page width for the first row of icons
        \vspace{-\baselineskip} % Required for vertically aligning minipages

        % The first parameter is the FontAwesome icon name, the second is the box size and the third is the text
        % Other icons can be found by referring to fontawesome.pdf (supplied with the template) and using the word after \fa in the command for the icon you want
        \icon{MapMarker}{12}{London, UK}\\
        \icon{At}{12}{\href{mailto:nico@dcotta.com}{nico@dcotta.com}}\\
    \end{minipage}
    \begin{minipage}[t]{0.275\textwidth} % 27.5% of the page width for the second row of icons
        \vspace{-\baselineskip} % Required for vertically aligning minipages

        % The first parameter is the FontAwesome icon name, the second is the box size and the third is the text
        % Other icons can be found by referring to fontawesome.pdf (supplied with the template) and using the word after \fa in the command for the icon you want
        \icon{Globe}{11}{\href{https://nico.dcotta.com}{nico.dcotta.com}}\\
        \icon{Github}{11}{\href{https://github.com/cottand}{github.com/cottand}}\\
        \icon{Linkedin}{11}{\href{https://linkedin.com/in/ndcotta}{linkedin.com/in/ndcotta}}
    \end{minipage}

    \vspace{0.1cm}


%----------------------------------------------------------------------------------------
%	EXPERIENCE
%----------------------------------------------------------------------------------------

    \cvsect{Experience}
    \input{experience}

%----------------------------------------------------------------------------------------
%	EDUCATION
%----------------------------------------------------------------------------------------

    \cvsect{Education}
    \input{education}
    \vspace{-0.4cm}
    \cvsectdesc{Projects}{
        \hspace{0.027\textwidth}
        \href{https://nico.dcotta.com/projects}{see my full portfolio at nico.dcotta.com}
    }
    \input{projects}
%----------------------------------------------------------------------------------------
%	ADDITIONAL INFORMATION
%----------------------------------------------------------------------------------------


    \vspace{1em}
    \begin{minipage}[t]{0.62\textwidth}
        \vspace{-\baselineskip} % Required for vertically aligning minipages

        \cvsect{Technologies}

        \vspace{-0.5em}
        \textbf{Proficient in} JVM (Kotlin, Java), Gradle

        \para
        \textbf{Experience with}
        Go, Python, C, Haskell, Typescript, Elixir

        \para

        Nomad, Nix + NixOS, AWS (Lambda, DynamoDB EC2, S3, CodeDeploy, API Gateway)

        \para

        gRPC + protobufs, Kafka, Akka, PostgreSQL, Redis, Cassandra, APM, Docker, React,
        VueJS

        \para

        Object-oriented, Functional, Actor-oriented, and Procedural programming.
        \\

        \colorbox{accent}{\textcolor{white}{\MakeUppercase{\textbf{Hobbies}}}}~~
        I enjoy DJing as well as kitesurfing and rock climbing!
%        \vspace{-0.5em}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.35\textwidth}
        \vspace{-\baselineskip} % Required for vertically aligning minipages
        \cvsect{Languages}

        \vspace{-0.5em}
        \textbf{English} - bilingual (C2 certification)\\
        \textbf{Spanish} - native\\
        \textbf{French} - bilingual\\
        \textbf{Portuguese} - advanced level\\

        \cvsect{Skills + Feedback}

        \vspace{-0.5em}
%        "Cool under pressure"\\
        "Pleasant to work with"\\
%        "Owns solutions and sees them through"\\
        "Aware of product quality"\\
        "Knows which battles to pick"\\
        I can google well!
    \end{minipage}
    \vspace{1.2em}
    \hfill
%    \begin{minipage}[t]{0.35\textwidth}
%
%    \end{minipage}

%----------------------------------------------------------------------------------------

\end{document}
