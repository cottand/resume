...
#let icon(name, shift: 1.5pt, boxed: true) = {
  ...
  box(
    ...
    //height: 20pt,
    //width: 20pt,
    fill: if boxed { primary_colour } else { white },
    inset: (x: 2.6pt, y: 2.6pt),
    //outset: 2pt,
    radius: 0pt,
...
#let vantage(
  ...
  set list(
    indent: 0em,
    body-indent: 0.6em,
    marker: text(fill: primary_colour, weight: "bold", "•")
  )
  set page(
    margin: (top: 1cm, bottom: 0.8cm, left: 1.6cm, right: 1.6cm),
  )
